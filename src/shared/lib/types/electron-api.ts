...
/**
 * Интерфейс полезной нагрузки для получения новых записей из Reddit.
 * Используется при запросе новых медиаконтентов в конкретном канале после определённого идентификатора.
 */
export interface RedditReceiveNewRecordsPayload {
  /**
   * Идентификатор последней полученной записи (параметр `after` из API Reddit).
   * `null` означает, что запрашиваются первые записи (без ограничения по `after`).
   */
  after: string | null;

  /**
   * Название канала (сабреддита), из которого требуется получить новые записи.
   */
  channel: string;
}

/**
 * Интерфейс ответа с новыми записями из Reddit.
 * Содержит список превью медиаконтента, а также параметры для пагинации.
 */
export interface RedditResponseNewRecordsData {
  /**
   * Массив превью медиаконтента.
   */
  records: MediaSummaryPreview[];

  /**
   * Идентификатор последней записи в ответе для продолжения выборки (`after` параметр).
   * `null`, если записей нет или достигнут конец выборки.
   */
  after: string | null;

  /**
   * Название канала (сабреддита), из которого получены записи.
   */
  channel: string;
}

/**
 * Интерфейс полезной нагрузки для отправки коллекции (альбома) медиаконтента из Reddit.
 */
export interface SendRedditCollectionPayload {
  /**
   * Уникальный идентификатор коллекции (например, ID поста или сессии).
   */
  id: string;

  /**
   * Объект, представляющий коллекцию медиа-альбома.
   */
  collection: MediaAlbum;
}

/**
 * Интерфейс полезной нагрузки для отправки превью одного медиа-элемента из Reddit.
 */
export interface RedditResponsePreviewPayload {
  ...
  /**
   * Превью-контент медиа-элемента, включая декодированный URL и метаданные.
   */
  preview: RedditMediaPreviewContent;
}

/**
 * Интерфейс, описывающий содержимое превью медиа-элемента Reddit.
 * Содержит информацию для отображения и загрузки превью.
 */
export interface RedditMediaPreviewContent {
  /**
   * Декодированная строка URL (обычно base64 или URL-кодированный путь).
   */
  decoded: string;

  /**
   * Прямой источник (URL) изображения превью.
   */
  src: string;
